---
name: kixpower-new
description: "🚀 全新项目启动：访谈→Producer→Dev→L2→QA→L4→Producer 收尾。用法：/kixpower-new 或 /kixpower-new 项目名 技术栈"
triggers: [user]
---
...
### 阶段 1：Producer（run_subagent kixpower-producer）
prompt 要点：
...
### 阶段 3：Dev（run_subagent kixpower-dev）
prompt 要点：
...
### 阶段 5：QA（run_subagent kixpower-qa）
...
- 先运行 `skills/kixpower/scripts/validate-memory-backlog.sh <ROOT>`；失败不得收尾
...
- 受 kix-guards hook 拦截：commit_budget 由 task_sizing 派生（v5.0 公式 `dag_layers + strong_coupling_count + bug_reserve`，详见 TEAM_CONVENTIONS.md §Task Sizing），hard_cap=10 绝对上限 / 必须在 feature branch / 禁 force push / 禁 destructive SQL
