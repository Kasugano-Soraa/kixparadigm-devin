# Devin CLI/Desktop Capability Map — Devin CLI/Desktop 机制事实地图

> **定位**：机制事实层索引（对照 `DSH-ADAPTATION.md` 的工具名映射，本文件是能力面全景）。**不是规则**——不写「kix 应集成 X」这类结论，只记机制事实 + 标注过的判断。后续任何 kix×Devin CLI/Desktop 任务（改 preset、注册命令、集成矩阵、写 guards）先查本文件，避免重翻 172 个包。
>
> **来源**：Devin CLI/Desktop 安装包内 `node_modules/@deepseek-ai/dsh-*` 全部包的 README（优先 zh）。
> **方法**：12 组并行梳理（8 组 DeepSeek + 4 组 GLM 跨厂商），组级机制无跨厂商分歧。
> **更新条件**：Devin CLI/Desktop 版本升级或机制被实际任务验证/推翻时复核。
...
| Agent 循环 | dsh-agent / agent-loop / presets / agent-instructions | 接口/唯一具体循环/插件三层分离，门禁沙箱权限恢复全在 `agent/*` + `tools/*` 事件流水线的插件层；preset=config.json / AGENTS.md 目录，roster 常驻挂载一次，会话按 scope 父链加入；AGENTS.md 基线 + 嵌套发现，`</system-reminder>` 字面转义 |
...
> 判别信息尽量放在**模型本来就会读的地方**（工具描述/目录：bash 的 run_in_background、subagent 的 continuable、workflow 的显式语义均已自带引导）。本表只补**防误用**与**组合形态**——做 Devin CLI/Desktop 相关任务先查这里，不用背。

| 任务形态 | 正确机制 | 判别要点（防误用） |
|---|---|---|
| 单个长测试/构建 | 后台 job：bash/bash `后台运行（exec timeout:0 / run_subagent is_background）`（参数描述自带引导）→ `get_output` 取、`kill_shell` 停 | 返回 **job id**（如 bash-1） |
| 多个机械测试并行（无需分析） | **纯 shell 管道可表达 → bash 单命令**；需**工具调用组合/结构化逻辑/自验证** → `exec（脚本文件执行）` 一次往返（presentation mode:both） | 实测（2026-08-17）：纯统计 bash 更省 token（exec（脚本文件执行） 输入约 2×）、exec（脚本文件执行） 更省来回（1 次 vs 3 次）且一次写对率高（内嵌基线自校验）；**文件内容都不进上下文**是两者共同优点（优于逐次 tools.read 灌全文） |
| 多 agent 并发扇出+聚合（无依赖观察/取证，语义决策留主线程） | `exec（脚本文件执行）` 内 `Promise.all([tools.subagent(...), tools.run_subagent（profile="kixpower-reviewer-cross"，跨厂商模型）(...)])`，`return` 只回聚合摘要 | 探针实证（2026-08-18）：exec（脚本文件执行） 工具面=全部常驻工具（26 个，含 subagent/cross/workflow）；3 路异质单步 7.5s 全过门禁；子调用结果不回模型上下文（Code Mode 契约，档一 PTC 条目）。**公平 E2E 修正（同日 WSL2 headless，preset diff=0）**：3 个微小子任务时扇出墙钟**反而更慢**（75.5s vs 串行 34.8s）、非 cache token 更贵（11.3K vs 2.7K）——载体固定开销需子任务时长摊薄，**微小探测任务直派串行更优**；扇出稳赢的是主线程步数（父 2 步 vs 4 步）与 cache 读。候选分级与回收条件见 `orchestration-lessons.md` ⑥ |
| 浏览器自动化（爬取/截图/E2E/批量表单/接管真实浏览器） | **原生插件 `browser{action}`**（kix-browser.js，2026-08-18 落地，同日扩到 17 action + 渐进披露合规）：open/snapshot/text/click/type/press/select/hover/back/forward/reload/wait/screenshot/upload/tabs/dialog/close；CDP attach 优先（KIX_BROWSER_CDP，接管真实浏览器登录态）+launch headless 兜底；会话跨调用持久；弹窗默认驳回+dialog{auto} 策略+lastDialog 回报。**渐进披露形态**：yml 挂载行默认注释（零常驻 schema 税）——kix-focus ACTIVATABLE_TOOLS.browser（pkgPath 本地解析）+ browser-native 目录组，capability_call 首用自动激活，kix_tool_deactivate 卸载；常驻=取消 yml 注释重启 | **已落地并 E2E 实证**：单测 12/12 + kix-focus 111/111（含 pkgPath 激活路径集成断言 + 枚举防线同步）；E2E 真浏览器 11 步全绿（open→wait→snapshot 元素提取→click 真实跳转→back/forward/reload 闭环→tabs→dialog→screenshot→file:// 拒绝→close）。**故意排除（范式红线/低频）**：页内任意 JS evaluate（blast-radius 红线，需要时 bash 直驱脚本=代码级可审查）；网络拦截/cookie/PDF/拖拽（脚本路径）。坑实录：①`$$eval` 传字符串函数体在该版 playwright-core 返回 undefined → 必须传真实函数引用；②重构时 open 执行分支误置于会话门禁后 → open 被自己拦死（E2E 抓出，单测盲区=只测校验路径不测成功路径）；③**restrict 裁不掉本层自有注册**（dsh-tools 源码定谳：restriction 只过滤继承面）——preset 层插件的渐进披露必须走 ACTIVATABLE pkgPath 路径，不走 restrict；④ACTIVATABLE 新增条目必须同步 activate/deactivate 工具 description（枚举防线会拦）；⑤playwright-core 懒 require+KIX_BROWSER_CORE 显式路径（npx 缓存可逐出）；⑥宿主重启后才装载/生效。playwright-core 安装位：宿主 home `npm install playwright-core --prefix ~/.config/devin` 或 KIX_BROWSER_CORE 指路 |
...
| 后台子代理（continuable） | `subagent 后台运行（exec timeout:0 / run_subagent is_background）` → `subagent panel / read_subagent` / 结算通知唤醒 | 返回 **durable subagent id**（不是 job id）——**勿用 `get_output` 查**（实测误用报 unknown job） |
| 组合：后台 job + workflow | ① 先 `run_in_background` 启动长任务 → ② workflow（前台阻塞，不阻塞 job）→ ③ `get_output` 回收 | 顺序固定：先后台 → workflow → 回收；回收由完成通知驱动，勿轮询 |
...
- **subagent id ≠ job id**：`subagent run_in_background` 返回 durable agent id，用 `subagent panel / read_subagent`/结算通知管理；`get_output` 查它会报 `unknown job`（实测 2 次，均自愈无害——收到 unknown job 后正确改等结算通知）。
...
8. **用户命令平面**：kix 的 /kixpower-* 命令注册为 Devin CLI/Desktop 原生命令 → 触发零 token 不消耗模型轮次。
...
| P0 | 档一 2 工具门禁链完整接线（kix-guards v3，2026-08-15） | ✅ 代码/测试/挂载已验；⚠️ 真实拦截 E2E 待重启后复验 v3 | 源仓库 `dsh/preset/plugins/kix-guards.js`（v3）：在原有 5 道门禁上补全 blast-radius 未接部分——commit budget（reflog 计数，hard cap 10 / progress.md 预算 / 冷启动 3）、feature branch 检查（真实读分支）、force push 完整检测（**修复 v1 静默失效**：`\b--force\b` 前无词边界永不匹配）、MCP GitHub 远程写保护（main/master deny / 无 branch deny / mutation ask）、终端数据库客户端保守拦截、UPDATE without WHERE、人类确认点 ask（reset --hard/clean -f/branch -D/stash drop/checkout --/restore/普通 push，Devin CLI/Desktop `{kind:'ask'}` → approval 服务，无 agent 自动降级 deny）。**独立审查（2fed9f16，对照 blast-radius ps1 + dsh-tools 契约）发现 3 漏拦 + 7 误伤，v3 已全部修复**：git 子命令解析式检测（`git -C/-c/git.exe` 不再绕过整个门禁）、isDestructiveSql 语句级+剥注释+大小写（`DELETE FROM a; SELECT...WHERE`、`UPDATE public.users SET`、`/* WHERE 1 */`、大写 SQL 不漏拦）且限定 SQL 上下文（grep/echo 不误伤）、force/main 检测限定真实子命令（commit message 含 "push +5"/"main.rs" 不误伤）、`--force/--mirror` 补 `(?<![\w-])`（`abc--force` 不误伤）、移除 rebase 兜底硬 deny（`rebase -i`/`pull --rebase` 放行，与 ps1 一致）、targetsControlPlane 限定用户级根（项目 settings.yaml 不误伤）、SQL 工具入白名单（门禁 4 死代码消除）、exec（脚本文件执行） 补 fs 直写检查。单元回归 **128 组全过**（含全部审查反例）；安装脚本 `install-kix-p1.ps1` 已纳入同步（幂等）。**如实声明**：当前 Devin CLI/Desktop 进程的 kixparadigm standing generation 是 v1 时代组装的，运行中不重读磁盘 → v2/v3 新增门禁须**重启 Devin CLI/Desktop 进程**后新会话复验（§5 项 6 挂账）。另修复 P0 工程缺口：源仓库此前无 kix-guards 副本（重装 preset 即丢门禁），现已纳入 `plugins/` + README 恢复清单第 5 项。**保持不接**（规则是负债 + 0% 误报）：角色边界（Producer/Orchestrator 源码黑名单，exec.agent 无角色标记）、预算一致性软警告与 progress/plan 开关（建议类不进机械层）、档二 10 tools/result 观察器（workflow schema 已是机械强制，重复机制=负债）、SQL 文件引用检查（psql -f，文本纪律覆盖） |
| P1 | 8：/kixpower-* 注册为 Devin CLI/Desktop 原生命令 | ✅ 已落地（安装 + E2E 已验） | 插件源 `dsh/preset/plugins/kix-commands.js`；安装脚本 `skills/kixpower/scripts/install-kix-p1.ps1`（用户 shell 运行，复制插件 + 幂等追加 config.json / AGENTS.md 挂载行，已实测幂等）。**模型无法直接写** `~/.config/devin/.config/devin 插件目录/`：kix-guards CONTROL PLANE 门禁实测拦截（design intent，防 prompt injection 篡改用户级配置；不绕过）——2026-08-15 由用户显式委托执行安装脚本。E2E：`standingKeyFor('kixparadigm')` 挂载校验通过；新会话敲 `/` 见 5 个候选；触发 `/kixpower-new` 返回「已注入（4614 字符）」且模型按流程执行（访谈开始） |
...
| P3 | 12：cordis preset 做 kix 自研原型 | ✅ 方案已定（零配置） | 用 Devin CLI/Desktop 出厂 `cordis` agent preset 开新会话即得 cordis_* 工具集（config/agent-presets/cordis，随安装目录存在，无需改任何配置）；详见 §3.2 |
...
| 更新 | kix-guards v7→v9（v8 = v1.2.10 自审整改；v9 = v1.2.11 发布/评论等确认类门禁降为软约束，不再提问） | ✅ 代码/测试已验（当前 210 组断言，含事故回归用例）；已部署 kixparadigm + kixparadigm-en 双安装（hash 校验一致，含根 plugins/ 副本同步治 drift）；⚠️ 真实拦截 E2E 待重启 Devin CLI/Desktop 后新会话复验 | `dsh/preset/plugins/kix-guards.js` v7 commit budget 三重修复：① reflog 计数改 %gs 口径只数 commit 类条目（reset/merge/pull/checkout/rebase 不计——不惩罚推荐的历史修整工作流；amend 只进 hard cap 口径、不进 budget 口径）；② marker 指向的 sprint 已有 done.md → warn 并回退最大编号目录，最大编号也已完结 → staleAll 标注进 deny 消息；③ 预算兜底链 progress.commit_budget > plan.task_sizing.derived_commit_budget > plan.blast_radius.max_commits > 冷启动 3（落冷启动必 warn），且显式 commit_budget=3 不再被「值等于默认」判断误覆盖（v6 潜伏优先级 bug）；deny 消息注明预算来源与 sprint 目录，移除误导性「按 DAG 重算」措辞。仓库侧配套：dae 开 sprint-10（hotfix 记账，commit_budget=8）+ marker=10 |
| 更新 | kix-discipline（2026-08-16，插件化改造 P0）：需求三检/验证 gate 机制化 | ✅ 代码/测试已验（当前 68 组断言）；⚠️ 端到端挂载 E2E 待重启后新会话复验 | `plugins/kix-discipline.js`（2026-08-16 新增，改造总览见 `PLUGINIZATION-ROADMAP.md`）：`tools/pre-execute` 实现编辑前查需求三检契约（spec）在档——无 spec + 首次实现编辑 → remind（默认，放行+注入提醒一次）/ ask（聊天内提问）/ block（deny），测试文件永远放行；`在工作区写 kix-discipline/spec.md 契约文件` 工具记录 goal/xy/assumptions/path/acceptance 五字段契约写工作区 `kix-discipline/spec.md`（跨会话可查）；`agent/turn-stopping` 回合结束有实现编辑无测试运行 → 注入「交付前验证三问」提醒（remindOnce）；`/kix-discipline` 命令 status/report/on|off。强度默认 remind（限制越少越好，字面明确任务可忽略），ask/block 需 config.json / AGENTS.md 该行 config 显式配置。边界：按 agent scope 挂载不覆盖子代理会话（同 kix-guards） |
| 更新 | kix-orchestration（2026-08-16，P2 补缺；v8 负向语义防误报；**v11 plan 契约**）：编排交接门禁机制化 | ✅ 代码/测试已验（当前 **89 组断言**）；⚠️ 端到端挂载 E2E 待重启后新会话复验 | `plugins/kix-orchestration.js`：`tools/pre-execute` 对 subagent* 分派携带 `current_sprint` 时校验工作区 `docs/.kixpower-current-sprint` marker 一致、`docs/sprint-N/{plan,progress}.md` 存在、progress 无 blocker、切 QA 时 completed==total；**v11：写 `docs/sprint-N/plan.md`（write 全量）校验预算链字段（`task_sizing.derived_commit_budget` / `blast_radius.max_commits`，缺则预算静默落冷启动 3）+ 任务清单存在性，默认 remind，独立提醒槽**；默认 remind / ask / block 可配；`/kix-orchestration` 命令。validate-handoff.ps1 的有界移植（深度 worktree/SHA/manifest 校验不移植，绑定 Devin 分派格式）；对应 DSH-ADAPTATION.md §2 hooks 现状更新 |
| 更新 | kix-focus（2026-08-16，P4 三层递进）：极简 + 渐进披露 + PTC | ✅ 代码/测试已验（当前 **123 组断言**；jobs 与 reviewer/dev/qa 常驻，lite/thinker/vision/fork/goal 首次使用自动激活：capability_call 代理即挂载；**2026-08-17 部署 E2E 坐实**：lite 平台条件化 shell、编曲成员分派 current_sprint 机械注入（sprintInjected））；端到端挂载已复验（WSL2 devin @33236，v1.2.13） | `plugins/kix-focus.js`：Phase 1 `tools.restrict` 把每轮工具面从 ~85 个（~108KB schema，估算 ~30.8K token）裁到常驻核心集（当前 deny 模式裁全局 MCP/web_search；历史 RESTRICT_ALLOW 仅作统计（10 个全局基础工具，不含 web_search）；subagent 五档与 kix_capability_* 是 scope 注册自动可见——**restrict 对 scope-local 名会 fail，故不列入 allow**），MCP/goal/cordis_* 按需，workflow/job_*/reviewer/dev/qa 常驻，`tools/change` 重试，量化 **-81.6%**（`scripts/quantify-focus.cjs` 可复跑）；Phase 2 `skill（Devin 技能目录）`（全局视图 `schemas(undefined)` 列出被裁剪工具，分组元数据不含全 schema）+ `直接调用对应工具`（agent 视图优先否则全局；被 restrict 的 MCP 省略 agent 走全局 execute，否则 UNKNOWN_TOOL；GitHub 写由 kix-guards 外层 unwrap）；**感知设计（2026-08-16 修订）：不挂 pre-execute deny**——restrict 已保证模型不可见被裁剪工具（直呼=UNKNOWN_TOOL 到不了 pre-execute），且 capability_call 内部子调用必须放行，引导由 call 返回与 persona 触发句承担。Phase 3 保持 `mode: both`（native 验证 + exec（脚本文件执行） 机械），kix「验证/观察 native 直呼」红线不变。配置：`enableRestrict:false` 关裁剪 / `extraResidentTools` 追加。**v1.2.13（2026-08-17）**：lite 档 `ACTIVATABLE_TOOLS` 快照 toolFilter 平台条件化（修 v1.2.12 双源漏改，部署 E2E 实锤）；编曲成员（qa/dev/reviewer）capability_call 分派注入 `current_sprint: N`（工作区 marker 驱动，交接 gate 机械兜底，见 §5 项 8）。端到端 schema 字节对比需重启后新会话实测 |
| 更新 | kix-consistency（2026-08-17，P5）：一致性守护写时拦截 | ✅ 代码/测试已验（当前 **36 组断言**）；⚠️ 端到端挂载 E2E 待重启后新会话复验 | `plugins/kix-consistency.js`：`scripts/check-dsh-consistency.cjs` 拆核为 `plugins/consistency-lib.cjs` 纯函数核心（CI 脚本与插件共用单一事实源）；pre-execute 对写 `dsh/preset/`、`en/preset/`、README*、package.json*、vision-bridge 相关文件按路径跑相关子检查（persona 预算 / 插件对同步 / memories 计数 / README 表述 / 版本对 / 单文件语法），失败默认 remind（ask/block 可配，remindOnce 每会话每类别一次）；仅源仓库指纹工作区触发，其余零开销放行。插件名清单动态化（`pluginNames()` 读目录）——新增插件自动纳入 CI 检查 |
...
- 新增厂商两步法（DSH-ADAPTATION §3 已写）：settings.yaml `llm-pi-ai.providers` 加 profile（apiKeyEnv 引用密钥，绝不写明文）+ preset config.json / AGENTS.md 加对应 subagent 工具行（agentOptions.provider/model 覆盖路由，已实证）。

### §3.2 cordis preset 原型切换（P3-12）

- **零配置路径**：Devin CLI/Desktop 出厂自带 `cordis` agent preset（安装目录 `config/agent-presets/cordis/`），新建会话时选择它即获得 `cordis_*` 五工具 + 组合编写技能。**不修改 kixparadigm preset**（默认组合保持无 cordis 工具 = 安全面最小）。
...
- 自研 kix 时亦可临时 patch 到 kixparadigm（需用户手动改 config.json / AGENTS.md，见 P1-8 同款门禁约束）。
...
1. ~~「kix 的 slash command 注册为 Devin CLI/Desktop 原生命令」的收益假设~~ → **P1-8 已落地并 E2E 坐实**（2026-08-15）：插件源 + 安装脚本 + 单元回归（6 组断言）+ GLM 独立审查（✅ 可安装，0 blocking）+ `standingKeyFor` 挂载校验通过 + **实机 E2E**：新会话敲 `/` 见 5 个 kixpower 候选；触发 `/kixpower-new` 返回「已注入 kixpower-new 流程（4614 字符）」且模型下一轮按流程执行（阶段 0 访谈开始）。
2. 「结构化输出 schema 化能替代 kix 验证 agent 的自觉纪律」——需在真实验证任务中对比误报率（P2-10 模板已就绪，待首个真实 workflow 用例验证）。
3. 「ralph 限发散阶段」——需一次真实发散/收敛任务对照实验。
4. 「workflow 脚本化 kix 团队 vs 手动 subagent 分派」的产出质量差异——**部分验证（2026-08-18 WSL2 headless 公平对照，preset diff=0）**：同任务双路径，产出质量等价（三项取证全对）；差异在成本结构——workflow 赢主线程步数（父 2 步/1 次调用 vs 4 步/4 次）与 cache（58.4K vs 72.8K），手动串行赢墙钟（34.8s vs 75.5s）与非 cache token（2.7K vs 11.3K）于微小子任务。**粒度条件**：子任务数十秒级时 workflow 墙钟才回本。⚠️ **方法学铁律：此类对照仅在 preset 同步部署后有效**——旧部署上同对照得出相反结论（扇出快 43%），实为漂移插件拖慢手动路径的伪影（4×墙钟差）。
5. 档一 1-7 的「直接依赖零改造」声明——每项在首个匹配任务中验证后即视为坐实。
6. ~~**kix-guards v3 新增门禁的真实拦截**~~ → **E2E 坐实（2026-08-17，WSL2 devin @33236 最新修复版）**：原 2026-08-15 挂账（128 组 → 142 组单元回归全过，但 standing generation 为 v1 时代组装）。最新部署（v9/v1.2.11）新会话实测：**push origin main 拦截**（禁推 main/master）、**push --force origin main 拦截**（force 重写历史）、**控制平面写拦截**（~/.config/devin/.config/devin 插件目录/ 新建文件也算编辑）、**exec（脚本文件执行） 受限能力拦截**（child_process 静态扫描）、**gh repo delete 拦截**（破坏性删除）；正向对照 3 项全放行（exec（脚本文件执行） 纯算术 / 只读 git / 只读 gh）——**5 拦 3 放，0 误报，无绕过通道**。另实测 kix-discipline 上下文注入（编辑后补验证提醒）与模型无绕过行为。**v10（2026-08-17，本轮源码迭代）**：修复 commit-on-main 检查的仓库根解析边界——`cd <repo> && git commit`（无 -C）+ 会话 cwd≠仓库根时 resolveRepoRoot 解析失败致检查静默跳过；新增命令位 cd 提取（`(?:^|[;&|]\s*)cd`，`echo cd /tmp` 不误匹配），非仓库目录 gitRead 失败自然 fail-safe。单元回归扩至 **213 组全过**（含新增 3 组 cd 用例）。**v10.1（2026-08-17，部署复验第二轮实锤）**：v10 只修了 resolveRepoRoot，**整链仍然不拦纯 commit**——`DANGEROUS_GIT` 自开源起就缺 `commit`（只有 commit-tree），`isGitWrite()` 对 `git commit` 返回 false → 2b 门禁整体跳过 → checkGitCommit（分支/预算检查）对普通 commit 永不执行 → **main 分支直接 commit 静默放行**（v1.2.12 的 E2E 声称未覆盖此路径；单测只测 repoRootFromText 未测整链）。修复 = 补 `commit` 进 DANGEROUS_GIT + **真实 git 仓库整链回归 6 组**（cwd=仓库根 main deny / cd 进入 main deny / -C 显式仓库 deny / feature 分支 allow / 非仓库 cwd fail-safe allow / message 含 push·main 不误拦），单元回归 **219 组全过**。部署新会话复验：`cd /root/kix-guards-e2e && git commit`（main）→ **BLAST RADIUS deny「禁止在 main 分支直接 commit」**，模型不绕过（提问用户选择合法路径）——与旧版放行形成对照（23d0f02 曾成功提交）。

7. ~~**lite 档 Linux 环境配置缺陷**（2026-08-17 前轮挂账：run_subagent（profile="subagent_explore"） 在本 Linux 部署不可用）~~ → **已修复并 E2E 坐实（v1.2.13）**：根因是**双源未同步**——config.json / AGENTS.md 的 tool-subagent-lite 行与 kix-focus.js `ACTIVATABLE_TOOLS.run_subagent（profile="subagent_explore"）.config` 快照各持一份 `toolFilter.allow`，v1.2.12 只把 preset 行平台条件化（`!!js process.platform === 'win32' ? 'bash' : 'bash'`），**快照仍硬编码 bash** → 模型走 `直接调用对应工具` 首次使用自动激活（capability_call 挂载用的是 ACTIVATABLE_TOOLS 快照）→ tools.restrict() 报 unknown global tool "bash"（部署 E2E 复验①首轮实锤）。修复 = 快照同步平台条件化（`process.platform === 'win32' ? 'bash' : 'bash'`，与 preset 行同源）。部署新会话复验：`run_subagent（profile="subagent_explore"）` 经 capability_call 自动激活 + 直接使用对应工具

8. ~~**交接 gate 依赖模型自觉带 `current_sprint: N`**（2026-08-17 前轮挂账：Tri-Block 分派常漏契约行 → 门禁静默放行）~~ → **机械层兜底已落地并 E2E 坐实（v1.2.13）**，双保险：
   - **kix-focus 注入兼容路径**：qa/dev/reviewer 已常驻直呼；旧 prompt 经 `直接调用对应工具` 分派时，工作区存在 `docs/.kixpower-current-sprint` 即自动把契约行注入/修正为 marker 值（纯函数 `injectSprintContractLine`/`readActiveSprint`，返回值带 `sprintInjected` 供模型感知）。部署 E2E 复验：模型按指令**故意省略契约行与 Sprint 字样** → capability_call 返回 `sprintInjected: 1` → kix-orchestration 门禁按注入行触发（`docs/sprint-1/` 缺 plan/progress 的 remind notice）——机械兜底闭环实证。
...
- **token 预算对照（2026-08-15）**：命令通道绝对零 token（command/run+done 为 log-only，
  ...
  最坏 100 sprint ≈ 2-3K，Devin CLI/Desktop toolResultPruner 对超限输出自动裁剪兜底）；曾有的定时提醒
...
- **会话存储结构**：`~/.config/devin/sessions/--<workspace-path>--/<session-id>/session.jsonl.zstd`（zstd 压缩 JSONL，`zstdcat` 解读）；GUI 列表索引在 `~/.config/devin/storages/workspace.json`（工作区→sessionIds+archived 名单）与 `session_projcache.json`（每会话 rows：sessionStats/title/tokenUsage/sessionListMetadata 等）
...
- **多副本同步纪律**（同日自省实证）：根 memories 与 preset memories 是**相异变体**（preset 带 Devin CLI/Desktop 适配头），禁 cp 覆盖式同步——只做追加式编辑；部署侧从对应 preset 单向 cp。根因注记（cross 审计修正）：事故根因是**有知识无机械保护**（明知变体分叉仍无条件 cp），非「没核对」——纪律必须落在可执行禁令，不靠核对自觉
