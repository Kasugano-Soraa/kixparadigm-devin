# kixpower × Devin CLI/Desktop workflow 团队编排模板（P2-10 + P2-11 落地）

> 把 kixpower CEO 团队编排（Producer → Dev → QA）从「手动多次 subagent 分派」升级为
> **Devin CLI/Desktop `workflow` 工具的单次脚本编排**。同时落地验证 gate schema 化（P2-10）：
...
6. **worker-thread 隔离是「API 塑形非安全边界」**：脚本与子 agent 仍受 sandbox + 门禁约束；exec（脚本文件执行） 内分派同理。
