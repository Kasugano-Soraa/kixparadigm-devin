...
> **适用场景**：VS Code + Devin Chat 环境下的多智能体协作软件开发
...
Devin Chat 中输入 `/` 会自动列出可用 prompt。本编排提供 4 个核心快捷命令：
...
| **Turn** | 一次 `subagent`（Devin CLI/Desktop 无 agentName，prompt 注入角色 body） 调用（同一 stage 内可有多个 turn） |
...
orchestrator 用 `subagent`（Devin CLI/Desktop 无 agentName，prompt 注入角色 body） 工具，prompt ≤ 5K tokens。模板见 `kixpower-orchestrator.agent.md` 的「子 agent 调用模板」章节。

**Dev 模板示例**：
```
工具: subagent
profile: "kixpower-dev"
prompt: |
  项目：@ [ROOT] / Sprint 1
...
- id: B5
  ...
  target_rules:
    ...
    mechanical_links:                   # 机械关联，grep+read 动态查询
      - type: callers
        of: [src/cache/mod.rs]
```
...
```bash
bash skills/kixpower/scripts/verification-fidelity-check.ps1 -ProjectRoot <ROOT> -PrevSprint <N>
```
...
- 源码编辑用平台精确编辑工具（优先 `edit`；不可用时用最小 `edit`），禁 inline shell
...
$testInput = @{ tool_name = "bash"; tool_input = @{ command = "git push --force"}; workspaceFolder = (Get-Location).Path } | ConvertTo-Json
$testInput | Set-Content "$env:TEMP\kix-hook-input.json" -Encoding utf8
cmd /c "bash -NoProfile -File `"$HOME/.copilot/skills/kixpower/hooks/blast-radius-check.ps1`" < `"$env:TEMP\kix-hook-input.json`""
```
