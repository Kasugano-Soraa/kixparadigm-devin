...
> **2026-08-15 落地进展**：档 A（stalled 检测）已在 Devin CLI/Desktop 落地为 kixst 动态插件——命令形态
...
> 变成「Devin CLI/Desktop 上可选启用的本地能力」。

- 旧版把「从 GitHub Actions 调 Devin CLI/API」写成**已实现 v3.4**，实际从未验证可用
  （CLI 在 runner 上受限、API endpoint 是猜测、适配层三选一是占位）
- 新版 v5.7 显式降级为 `degraded-local`：workflow 只做能力报告（result `NOT_TRIGGERED`），
  不调用 Devin、不改 Sprint 文件、不创建 Issue/Review；升级 `enabled` 需项目级 adapter
