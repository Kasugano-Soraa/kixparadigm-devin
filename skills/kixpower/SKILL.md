...
## Devin 移植注记（本档唯一事实源补充）

- **机械门禁**：Copilot 时代的 `hooks/*.ps1` 与 DSH 的 `plugins/kix-*.js` 已由全局 `kix_guards.py` hook 接管（force push / main 直写 / 破坏性 SQL / gh 远端删除硬拦；commit 预算=结算提醒）。角色边界（QA 不写业务源码等）由 subagent profile 的 `allowed-tools` + prompt 约束承担。
- **分派**：`runSubagent agentName:"X"` → `run_subagent(profile="X")`；orchestrator 不建行——协调留主线程（本文档全部编排规则作用于主线程）。
- **模型钉名**：文档中的 `model: "GLM-5.2"` 等字符串不适用；跨厂商异质观察用 `kixpower-reviewer-cross` profile（其 `model:` 字段钉异厂商模型，可改）。
- **脚本**：`new-sprint.sh` / `validate-memory-backlog.sh` 已移植 bash/python 版（在 `scripts/`）；`verification-fidelity-check.ps1`（320 行覆盖率量化）未移植——按 TEAM_CONVENTIONS §verification fidelity 口径手动度量，或后续补 .sh。
- **识图**：`read` 直接读图片文件；浏览器用 `browser_preview`。
- **其余 DSH 专属机制**（kix-focus 工具面裁剪、kix-route 运行时路由、goal/workflow 工具、capability_call）在 Devin 无对应，相关段落仅作机制考古参考。


> **Devin CLI/Desktop 适配注记**：本文件从 Devin 导入。工具名/机制映射（run_subagent→subagent、exec→bash、ask_user_question→ask_user_question、hooks 不自动触发（已由 kix-guards 原生替代）、跨厂商模型字符串不适用（用 `run_subagent（profile="kixpower-reviewer-cross"，跨厂商模型）` 工具行）、slash command 已注册为 Devin CLI/Desktop 原生命令）见 classic 档 `DSH-ADAPTATION.md`，冲突时以该文件为准。
...
> 常驻规则在 `config.json / AGENTS.md` persona「成本纪律」节（两 edition 均有）；本表是路由索引。
> 机制落地：`plugins/kix-cost.js`（lite 档路由回退默认休眠 + 思考强度归一化）+ 四档子代理工具行 + per-role 预算帽。

| # | 方法 | 落地段 |
|---|---|---|
| ⑭ | 机械档精简组合（`run_subagent（profile="subagent_explore"）`：机械 persona ~60 token + 只读 4 工具 + 8K 帽；每步固定开销 34.3k → ~5.9k，↓83%） | config.json / AGENTS.md `tool-subagent-lite` 行 |
| ⑮ | lite 档不钉路由（继承父代理路由；插件默认不含任何模型/厂商偏好）；仅当部署显式给 lite 钉 provider/model 时才首次请求探测 + 不可用回退环境默认路由 | `plugins/kix-cost.js` |
| ⑯ | 思考强度归一化：`run_subagent（profile 内 model: 钉深思考模型）`（≥98K 帽）→ max；其余 deepseek 子代理 → high（适配器默认 high/256K，64K 帽是跑飞防线） | `plugins/kix-cost.js`（agent/request waterfall） |
| ⑰ | per-role 预算帽：subagent/cross/fork 64K、thinker 128K、lite 8K | 各工具行 `agentOptions.maxTokens` |
| ⑱ | 禁轮询空转 / 观察者门控（1→分歧+1，并发≤3）/ [EFFORT]+[BUDGET] 标注 / outputSchema 回流 / 跨会话结论复用 / 同会话去重 | persona「成本纪律」节（config.json / AGENTS.md，P1 压缩为选择层；机制由 kix-cost/kix-route 插件强制） |
| ⑲ | 等待步骤零思考：get_output/subagent panel / read_subagent 等待是检查点不是思考点；后台任务运行期间做其他独立工作（实测：单次 get_output 等待烧 12,998 思考） | persona「成本纪律」节（config.json / AGENTS.md，同上） |
| ⑳ | review 流程阶段 2 取证分工：orchestrator 判断 + `run_subagent（profile="subagent_explore"）` 并行机械取证（只读/检索/核对/枚举走 lite，禁止 orchestrator 逐文件通读 diff） | `prompts/kixpower-review.prompt.md` 阶段 2 |
...
| # | 方法 | 落地段 |
|---|---|---|
| ㉑ | 主会话预算：动态窗口与 usage/tokenMeter 同时有效时按窗口分档 token 预算（≤128K→0.85、≤400K→0.65、≤1M→0.40、>1M→0.35）hard gate；任一计量面缺失时才回退 150K/step 41，超线必须完成 foreground lite/goal 交接 | `plugins/kix-budget.js` + config.json / AGENTS.md |
...
| ㉕ | 马拉松交接：context 或 fallback-step gate 强制主线程调用 foreground `run_subagent（profile="subagent_explore"）` / `todo_write / docs 落盘（Devin 无 goal 工具）`；结构化成功 envelope 才解锁，报告正文引用失败样例不误判 | `plugins/kix-budget.js` + goal/continue |

## v5.12 子代理编排面与激活抖动（2026-08-17 账本 + 2026-08-18 误伤复盘）

> 107 个子代理会话 / 2,432 次调用 / 162.5M 重读 token——与主会话同量级的另一半燃烧。首步固定开销中位 23.4K（system 38.8K chars + tools 34.3K chars）；**system 里 ~25K chars 是 exec（脚本文件执行） 的 TypeScript 工具镜像**（每个工具 schema 付两次钱）；40/107 个子代理尝试过再分派（50 次）——每个孙代再付一次全额固定开销；`change` 请求头（工具面中途变更）后 cacheRead 归零、整个上下文全价重读。

| # | 方法 | 落地段 |
|---|---|---|
| ㉖ | 有界 evidence delegation：depth-1 仅可派 `run_subagent（profile="subagent_explore"）` 机械取证；in-process `maxDepth:2`、静态/动态 9-name toolFilter 与 `kix-cost` proxy-target guard 共同拒绝 depth≥2、regular/cross/goal/workflow；文件/执行/检索/jobs/skill/report/MCP 保留 | config.json / AGENTS.md + `plugins/kix-focus.js` + `plugins/kix-cost.js` |
