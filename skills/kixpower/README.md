...
- **L1 Agent Loop**: `subagent`（Devin CLI/Desktop 无 agentName，prompt 注入角色 body） 调用 Dev/QA
...
| v5.10 | 2026-08-17 | **子代理编排面与激活抖动**（107 子代理/162.5M 重读账本实测）：㉖ 编排面裁剪（deny 名同时从 tools 数组与 system TS 镜像消失；exec（脚本文件执行） 为保留传输名不可 restrict、正当执行通道保留；静态 deny 恒注册名 + kix-cost child guard 兜底条件挂载名）㉗ kix_tool_deactivate 延迟到回合末（change 头缓存重置清零）㉘ 子代理会话吃 v5.9 预算/compaction 覆盖。落地 config.json / AGENTS.md 各 tool-subagent 行 + kix-cost/kix-focus。 |
...
| v6.1 | 2026-08-18 | **门禁信号降噪**：动态计量有效时仅 token 预算 hard gate，step 41 只作计量缺失 fallback；handoff 只认 canonical 顶层 envelope；exec（脚本文件执行） 剥离明确数据语法，regex/division/tagged-template 歧义按原文 fail-closed；depth-1 仅 lite，maxDepth=2 + toolFilter + proxy-target guard 阻断递归/goal。 |
