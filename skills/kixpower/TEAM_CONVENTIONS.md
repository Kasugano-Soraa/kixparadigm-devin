...
> **Devin CLI/Desktop 适配注记**：下表从 Devin 语境导入。Devin CLI/Desktop 映射：`execute`/`exec`→`bash`、`web`→`web_search`、GitHub MCP→`gh` CLI、无 grep+read；依赖/影响面分析用 grep + read。
...
| 无 grep+read；依赖/影响面分析用 grep + read。

### 源码编辑纪律

- **源码编辑首选平台提供的精确编辑工具**（有 `edit` 时优先；当前仅有 `edit` 时使用最小补丁）；**禁止用终端 inline 脚本编辑源码**（PowerShell / `python -c` / here-string），转义极易出错致文件损坏（有多次 `git checkout` 恢复案例）。
...
- `bash` >200 行预期 → 追加 `\| Select-Object -Last 100`
...
- 读 Devin 设置的 `github.copilot.advanced.models`
...
- **现象**：长命令 `gh api --method POST ... --input "$env:TEMP\payload.json"` 在 bash 终端的回显会被截断，残留片段（如 `d" -Raw -Encoding utf8`）看似命令未执行
- **误判反模式**：把"回显不完整"当成"未执行" → 重复 POST（PR#2980 事故根因）
- **正确判断**：`bash` sync 模式下，**exit code 0 + 无 stderr error 输出 = 已成功**。回显完整性是显示层问题，与命令执行状态无关
  ...
  target_rules:
    ...
    mechanical_links:
      - type: callers                 # 谁调用了本 task 改的 pub fn
        of: [src/cache/mod.rs]        # grep+read 自动查
...
替代 v3.1 的「机械关联手列」。Producer 只声明规则，grep+read 实际查询：

| type | 含义 | 查询 |
|---|---|---|
| `callers` | 谁调用了本 task 改的符号 | `grep+read（Devin 无 grep+read） to=<file>` 反向 |
| `callees` | 本 task 调用了谁 | `grep+read（Devin 无 grep+read） from=<file>` 正向 |
| `same_trait` | 同 trait 其他 impl | `grep+read（Devin 无 grep+read） trait=<name>` |
| `same_struct` | 同 struct 构造点 | `grep+read（Devin 无 grep+read） struct=<name>` |

**L2 Observe 时**：orchestrator 实际跑 grep+read 查询，把结果合入 in_scope（动态生成，不污染 plan.md）。
...
grep+read 是工作区级代码关系图工具。MCP server 配置在 `.vscode/mcp.json`，所有 agent 共享。**只在"需要时"用，不是每次都用**。

### 何时使用（必用场景）

| 场景 | 用什么工具 |
|---|---|
| 模块边界 / 包依赖 / 循环依赖分析 | `grep+read（Devin 无 grep+read）`、`grep+read（Devin 无 grep+read）` |
| 改一个文件前评估影响面（找出谁依赖它） | `grep+read（Devin 无 grep+read）`（filter `to=<目标文件>`） |
| 多跳可达性："这个改动会沿着 import 链传到哪里" | `grep+read（Devin 无 grep+read）`（from→to） |
| 找符号定义、跨文件引用证据 | `grep+read（Devin 无 grep+read）` |
| Producer 评估"重构涉及多少文件"、QA 评估回归范围 | `grep+read（Devin 无 grep+read）` + `grep+read（Devin 无 grep+read）` |
...
1. **先 status**：`grep+read（Devin 无 grep+read）`（参数 `path` 留空 = 当前工作区）。返回 `cache=missing/stale` 才需要 index。
2. **按需 index**：`grep+read（Devin 无 grep+read）`（耗时几秒到几十秒，取决于工作区大小）。**禁止每次都 index**——索引未失效时直接查询即可。
3. **失败兜底**：若 MCP 工具调用报错（如 server 未启动、缓存损坏），**不要反复重试**——降级为 `grep` / `read`，并在 progress.md 记一条 "grep+read 不可用：\<原因\>"，继续主干任务。
...
### hook 测试方法：用 cmd 重定向，不用 bash 管道

- **bash 管道** `Get-Content file.json | & bash -File hook` 传**对象**（string 数组），子进程 `$Input` 枚举行对象而非字节流 → JSON 首字符可能被污染，`ConvertFrom-Json` 失败、hook **静默退出**。
- **cmd 重定向** `cmd /c "bash -File hook < file.json"` 传**字节流**，子进程 `$Input | Out-String` / `[Console]::In.ReadToEnd()` 正确读取 → **这是 VS Code hook 的实际调用方式**。
- **测试 hook 必须用 cmd 重定向**（或 `[Console]::In.ReadToEnd()`），禁止用 bash 管道。
- 案例：`blast-radius-check.ps1` 在 bash 管道测试下静默退出（JSON 解析失败 position 0），被误判为"hook 失效"；实际 VS Code 字节流 stdin 下一直正常——是**测试框架错了**，不是 hook 错了。
